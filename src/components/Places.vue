<template>
  <h3>Наши площадки</h3>
  <div class="events">
    <div class="event" v-for="place in places" :key="place .id">

      <img :src="place.foto_file" alt="Image" class="event__image" />
      <div class="texts">
        <div class="event__title">{{ place.name }}</div>
        <div class="event__description">{{ place.description }}</div>
      </div>
<!--      <RouterLink :to="'/events/' + place .id"><div class="more"> Подробнее... <i class="pi pi-window-maximize" style="font-size: 1rem"></i></div> </RouterLink>-->
